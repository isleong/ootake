	...
	Uint16		color)
{
	...
	if (lock_screen256_surface(&pitch, &pPixels))
	{
		...
		for (i = 0; i < 224; i++)
		{
			...
			{
				for (j = 0; j < 256; j++)
					*pPixels++ = color; //2byteぶん(１ドット)書き込まれる。
			}
			else
			{
				for (j = 0; j < 256; j++)
	...
	byte	b) //v2.73更新。任意の色でクリアできるようにした。※16ビットモードのときはモノクロ以外だと色化けするので現在はモノクロ固定でrのデータのみ使用。
{
	...
	if (_BitsPerPixel == 16)
		color = (r >> 4 << _Rshift) | (r >> 4 << _Gshift) | (r >> 4);
	else
		color = r*256+r;
	SCREENDD_FillRect(0, 0, 256, 224, color);
	zoom_ddTensou256(); //SCREENDD_FillRect()で作ったクリアイメージをプライマリサーフェスへ転送する。
}
