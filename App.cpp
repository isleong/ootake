...
static char		_SoftVersion[5] = "2.75"; //Kitao追加。バージョン情報。5バイト(4文字)固定。
...
static Sint32	_ScanLineType			= 1; //Kitao追加。0…ノンスキャンライン，1…縦横スキャンライン，2…横のみスキャンライン，3…縦のみスキャンライン(未実装)，4…TVモード。5以降も拡張可能。v2.63からデフォルトを1に。
...
first_run_cpu()
{
	...
	if (_PowerOnEffectType != 0) //※_PowerOnEffectTypeが0の場合は、ゲームによって（パワーリーグ３など）数ミリ秒白画面が出る挙動で、古いテレビやRF出力の感じに。v2.75更新
	{
		...
		{
			SCREEN_Clear(n*16-12, n*16-12, n*16-12); //画面フィル
	...
	BOOL	screenUpdate)
{
	...
	if (screenUpdate)
	{
		MENU_CheckItem(_SizeMenu, WM_SCREEN_TV, _bTvMode);
		if (_bFullScreen)
			SCREEN_Clear(0,0,0); //フルスクリーンの場合、画面にゴミが残らないようクリアしておく
		if (_bTvMode)
		{
